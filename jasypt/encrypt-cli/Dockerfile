...
ENV INPUT_PASSWORD secret

COPY --from=builder /jasypt.jar /app/jasypt.jar

CMD java -Djava.security.egd=file:/dev/./urandom -cp /app/jasypt.jar org.jasypt.intf.cli.JasyptPBEStringEncryptionCLI input="$INPUT_PASSWORD" password="$MASTER_KEY" algorithm="$ALGORITHM"
